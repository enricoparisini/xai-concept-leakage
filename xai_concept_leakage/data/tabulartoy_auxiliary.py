...
def produce_data_gt_experiment(samples, cov=0., num_concepts=3, input_size=7, save_path = None):
...
def generate_tabulartoy_data(cov, n_samples, save_folder, split_ratios = [0.7, 0.2, 0.1], num_concepts=3):
    x, y, c = produce_data_gt_experiment(n_samples, cov=cov, num_concepts=num_concepts, input_size=7)
    ratio_1 = int((split_ratios[1]+split_ratios[2])*1e10) / 1e10
    ratio_2 = int(split_ratios[2]/(split_ratios[1] + split_ratios[2])*1e10) / 1e10
    x_train, x_test_temp, c_train, c_test_temp, y_train, y_test_temp = train_test_split(x, c, y, 
                                                                                    test_size=ratio_1, 
                                                                                        random_state=42)
    x_val, x_test, c_val, c_test, y_val, y_test = train_test_split(x_test_temp, c_test_temp, y_test_temp, 
                                                               test_size=ratio_2, 
                                                                   random_state=42)
...
def load_tabulartoy_data(save_folder, numpy = False, considered_concepts = ['0', '1', '2']):
    ...
    if numpy:
        return [x_train.to_numpy(), x_val.to_numpy(), x_test.to_numpy(), 
                c_train.to_numpy(), c_val.to_numpy(), c_test.to_numpy(), 
...
def TT_dataloaders(data_folder, num_workers = 1, batch_size = 512, considered_concepts = ['0', '1', '2'], c_logits = False):
    (x_train, x_val, x_test,
     c_train, c_val, c_test, 
     y_train, y_val, y_test) = load_tabulartoy_data(data_folder, considered_concepts = considered_concepts)
    
    if c_logits:
        c_train, c_val, c_test = [logits_from_probs(c) for c in [c_train, c_val, c_test]]
    
    x = torch.FloatTensor(x_train.to_numpy())
    c = torch.FloatTensor(c_train.to_numpy())
    y = F.one_hot(torch.Tensor(y_train.to_numpy()).type(torch.LongTensor)).squeeze(1).type(torch.float)
    ...
    y = F.one_hot(torch.Tensor(y_val.to_numpy()).type(torch.LongTensor)).squeeze(1).type(torch.float)
    y = y.argmax(dim=-1).cpu().detach()
    ...
    y = F.one_hot(torch.Tensor(y_test.to_numpy()).type(torch.LongTensor)).squeeze(1).type(torch.float)
    y = y.argmax(dim=-1).cpu().detach()

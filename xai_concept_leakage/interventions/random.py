...
class IndependentRandomMaskIntPolicy(InterventionPolicy):
    ...
    def __call__(
        ...
        if self.group_based:
            ...
            concept_group_map = dict([(i, [i]) for i in range(c.shape[-1])])
                ...
                [
                    [
                        (
                            1
                            / (
                                len(self.concept_group_map)
                                - prev_intervened_groups[idx]
                                if prev_intervened_groups[idx]
                                else len(self.concept_group_map)
                            )
                            if group_mask[idx, group_idx] == 0
                            else 0
                        )

...
def plot_y_c_accuracies(
    results, dl_label="test", checkpoint_names=None, y_lim=1.0, save_path=None
):
    scores_dict = extract_scores_from_results(
        results,
        test_dl_label=dl_label,
        score_labels=["y_accuracy", "c_accuracy"],
...
def plot_intervention_performance(
    results,
    policy="random",
...
def plot_model_vs_model_scores(
    results,
    dl_label="test",
    score_labels=["CTL", "ICL", "ois"],
    checkpoint_names=None,
    y_lim=1.0,
    legend_pos="auto",
    title=None,
    ...
    if relabel_score:
        ...
        score_labels = [
            relabel_score[old_label] if old_label in relabel_score else old_label
...
def plot_hard_scores(
    ...
    scores_dict = {
        dataset_label: {
            score_label: results[dataset_label][score_label]
            for score_label in score_labels
        }
...
def plot_CEM_scores(
    results,
    dl_label="test",
    score_labels=["y_accuracy", "CT_MI_mix"],
    checkpoint_names=None,
    y_lim=1.0,
    legend_pos="auto",
    title=None,
    ...
    if legend_pos != "auto":
        sns.move_legend(
            ax, "lower left", bbox_to_anchor=(legend_pos[0], legend_pos[1]), title=""
        )
    if y_lim is not None:
        ax.set(ylim=(0, y_lim))
    if title:
...
def plot_alignment_leakage(
    ...
    if ("_i" not in obs_label) and (obs_label not in extra_vector_scores):
        scores_dict_df = {
            k: [v.mean(), v.std() / np.sqrt(len(v) - 1)] for k, v in scores_dict.items()
        }
...
def unravel_c_vec(c_vec, n_samples, n_concepts):
    return np.swapaxes(
        np.array(
            [
                c_vec[i_c * n_samples : (i_c + 1) * n_samples]
                for i_c in range(n_concepts)
            ]
        ),
        0,
...
def fit_PCA_c_pos_neg(c_pos, c_neg, pca_dim=2, return_pos_neg_transformed=True):
    ...
    def pca_transform(c_vec):
        ...
        c_transformed = principal.transform(
            scaling.transform(normalizing.transform(c_ravel))
        )
        return unravel_c_vec(c_transformed, n_samples, n_concepts)

    if return_pos_neg_transformed:
        c_pos_pca = unravel_c_vec(
            c_pca[: n_samples * n_concepts], n_samples, n_concepts
        )
        c_neg_pca = unravel_c_vec(
            c_pca[n_samples * n_concepts :], n_samples, n_concepts
        )
...
def fit_PCA_c_mix(c_sem, n_concepts, pca_dim=2, return_mix_transformed=True):
...
def plot_pca_based_on_y_sns(
    ...
    if legend_pos != "auto":
        sns.move_legend(
            ax, "lower left", bbox_to_anchor=(legend_pos[0], legend_pos[1]), title="y"
        )
    if title:
        suptitle = plt.title(title)
        fdct = {"color": "black", "fontsize": title_fontsize}
...
def plot_pca_based_on_other_cs(
    c_mix_pca,
    y_true,
    c_true,
    i_c,
    ...
    plt.plot(
        ...
        markersize=4,
        linewidth=0,
        label=rf"$c_{i_other_cs[0]+1}=1, c_{i_other_cs[1]+1}=1$",
    )
    ...
    plt.plot(
        ...
        markersize=4,
        linewidth=0,
        label=rf"$c_{i_other_cs[0]+1}=0, c_{i_other_cs[1]+1}=1$",
    )
    ...
    plt.plot(
        ...
        markersize=4,
        linewidth=0,
        label=rf"$c_{i_other_cs[0]+1}=1, c_{i_other_cs[1]+1}=0$",
    )
    ...
    plt.plot(
        ...
        markersize=4,
        linewidth=0,
        label=rf"$c_{i_other_cs[0]+1}=0, c_{i_other_cs[1]+1}=0$",
    )
    if title:
        suptitle = plt.title(title)
        fdct = {"color": "black", "fontsize": title_fontsize}
...
def heatmap_groundtruth_interconcept_nMI(
    IC_nMI, title, textcolors=("black", "white"), save_path=None
):
    """

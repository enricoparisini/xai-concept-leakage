...
def means_and_SEs_eval_and_leakage(results, eval_score="random", leakage_score="CTL"):
    ...
    if eval_score in ["random", "coop"]:
        int_bool = True
        eval_scores_final_dict = {
            model_label: eval_scores[:, eval_scores.shape[-1] - 1]
    ...
    leakage_scores_dict = extract_scores_from_results(
        results, score_labels=[leakage_score]
    )

    eval_scores = list(eval_scores_final_dict.values())
    if int_bool:
        eval_means, eval_SE = 1 - np.mean(eval_scores, axis=-1), sem(
            eval_scores, axis=-1
        )
    else:
    ...
    leakage_means, leakage_SE = np.mean(leakage_scores, axis=-1), sem(
        leakage_scores, axis=-1
    )
    leakage_dist = np.array([leakage_means, leakage_SE]).T
    return eval_means, leakage_means, eval_SE, leakage_SE


def rubin_multiple_imputation_pearson(x_obs, y_obs, sx, sy, M=10000, rng_seed=None):
    """

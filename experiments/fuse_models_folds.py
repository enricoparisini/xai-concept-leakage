...
def model_names_fold_from_classes(checkpoint_classes, n_fold = 5):
    ...
    for checkpoint_class in checkpoint_classes:
        checkpoint_names += [checkpoint_class + "_fold_" + str(i+1) for i in range(n_fold)]
...
def fuse_folds(checkpoint_class, results_folder, 
               n_fold = None, checkpoint_names = None,
               observables = None, dl_labels = None,
               save_path = None):
    '''
    ...
    if n_fold is not None:
        checkpoint_names = model_names_fold_from_classes([checkpoint_class], n_fold = n_fold)
    ...
    for dl_label in dl_labels:
        ...
        for observable in observables:
            if observable in maps_dict:
                temp_dict = extract_scores_from_results(results, test_dl_label = dl_label, score_labels = [observable])
                dl_fusion_dict[observable] = np.array([ 
                    maps_dict[observable](rep_scores) for rep_scores in temp_dict.values()
                ])
